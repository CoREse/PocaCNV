    ...
    for record in vcf.fetch():
        try:
            ...
            if record.info["SVTYPE"]=="INS":
                temp=interval(record.chrom,record.pos,record.pos+abs(record.info["SVLEN"]),"INS")
...
def parse_my(filename):
    ...
    for line in myout:
        sl=line.split(",")
        temp=interval(sl[0].split(":")[0],int(sl[0].split("-")[0].split(":")[1]),int(sl[0].split("-")[1].split(":")[1]),"LOSS" if sl[2]=="DEL" else "INS")
        records.append(temp)
    myout.close()
    return records